<template>
  <div class="card h-100">
    <div class="card-header d-flex justify-content-between align-items-center">
      ...
      <button class="btn btn-sm btn-outline-theme" @click="showCreate = true">
        <i class="fas fa-plus"></i>
      </button>
    </div>

    <!-- Create Mode -->
    <div v-if="showCreate" class="card-body">
       ...
       <div class="mb-3">
         ...
         <div class="list-group" style="max-height: 200px; overflow-y: auto;">
            <button
                ...
                :class="{ active: selectedParticipantIds.includes(String(player._id)) }"
       ...
       <div class="mb-3">
           <input v-model="newConversationName" class="form-control form-control-sm" placeholder="Name (Optional)" />
       </div>
       <div class="d-flex justify-content-between">
           <button class="btn btn-sm btn-outline-secondary" @click="cancelCreate">Cancel</button>
           <button class="btn btn-sm btn-theme" @click="createConversation" :disabled="selectedParticipantIds.length === 0">Create</button>
       </div>
    </div>

    <!-- List Mode -->
    <div v-else class="card-body p-0 overflow-auto">
      <div v-if="chatStore.conversations.length === 0" class="p-3 text-muted text-center">
      ...
      <div class="list-group list-group-flush">
        ...
        >
          <div class="d-flex justify-content-between align-items-start">
             <div class="fw-bold text-truncate">{{ getConversationName(conv) }}</div>
             <small class="text-muted" style="font-size: 0.7em;">{{ formatTime(conv.updatedAt) }}</small>
          </div>
...
const potentialParticipants = computed(() => {
    return galaxyStore.players.filter(p => String(p._id) !== String(galaxyStore.currentPlayerId));
});

function getPlayerColor(player: any) {
    return PLAYER_COLOR_LOOKUP.get(player.color) || { background: '#ccc', foreground: '#000' };
}

function toggleParticipant(id: string) {
    if (selectedParticipantIds.value.includes(id)) {
        selectedParticipantIds.value = selectedParticipantIds.value.filter(pid => pid !== id);
    } else {
