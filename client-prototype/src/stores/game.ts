  ...
  actions: {
    async fetchGames() {
      ...
      try {
        ...
        this.openGames = gamesList.filter(
          (g: Game) => !g.userHasJoined && g.state.status === "PENDING",
        );
      } catch (err: any) {
    ...
    async fetchEvents(gameId: string) {
      try {
        const response = await axios.get(`/api/v1/games/${gameId}/events`);
        this.events = response.data as GameEvent[];
      } catch (err: any) {
        console.error(err);
      }
    },
