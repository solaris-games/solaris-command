...
export const useChatStore = defineStore("chat", {
  ...
  getters: {
    activeConversation: (state) =>
        state.conversations.find(c => String(c._id) === state.activeConversationId) || null,
  ...
  actions: {
    ...
    async createConversation(gameId: UnifiedId, name: string | undefined, participantIds: string[]) {
        try {
            ...
            const existing = this.conversations.find(c => String(c._id) === String(conv._id));
            if (!existing) {
    ...
    async fetchMessages(gameId: UnifiedId, conversationId: UnifiedId) {
        try {
            const res = await axios.get(`/api/v1/games/${gameId}/conversations/${conversationId}/messages`);
            this.messages[String(conversationId)] = res.data.messages;
        } catch (err) {
             console.error("Failed to fetch messages", err);
        }
    },
    async sendMessage(gameId: UnifiedId, conversationId: UnifiedId, content: string) {
        try {
            const res = await axios.post(`/api/v1/games/${gameId}/conversations/${conversationId}/messages`, {
                content
            });
    ...
    handleMessage(message: Message) {
        ...
        const exists = this.messages[conversationId].some(m => String(m._id) === String(message._id));
        if (!exists) {
            this.messages[conversationId].push(message);
        }

        const conv = this.conversations.find(c => String(c._id) === conversationId);
        if (conv) {
