...
- **Planets:** The economic engine. They generate **Prestige**.
  - _Capital Planets:_ These serve as **Root Supply Sources**. They function indefinitely and cannot be besieged out of supply (though they can be captured).
  - Capturing an enemy _Capital Planet_ is devastating, it could very well knock the opposing player out of the game.
...
- **The Crash Rule:** If two or more units attempt to enter the same hex on the same Tick, a **Collision** occurs.
  - _Scope:_ This applies to **ALL** units (Enemy vs Enemy, Ally vs Ally, or Self vs Self).
  - _Logic:_ The unit with the best **initiative** will win the hex. Tie-break on active steps, then total steps, then random.
  - _Outcome:_ Other units "bounce" (stay in current hex), expend **MP**, and suffer **1 Step Suppression**.
  - _Note:_ If a unit attempts to enter a hex that is already occupied (by units advancing in combat) then that unit will bounce (not the unit in the hex).

> **Scenario: The Race** > **Player A** (Frigates, High MP) and **Player B** (Destroyers, Low MP) both race for a neutral planet.
>
...
4.  **Advance on Victory (Blitz):**
    ...
    - _Note:_ This movement happens _before_ other units move in the current Tick.
5.  **Regrouping:** Both units enter a `REGROUPING` cooldown.
...
- **New Deployment:** Players spend Prestige to deploy new units.
  - _Rule:_ New units spawn adjacent to any player owned planet. **They spawn with all but one step suppressed.** They require supply cycles to become combat-ready.
...
- **Scrapping:** Players can decommission units or individual steps and small amount of prestige will be returned to the player.
