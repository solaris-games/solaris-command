...
describe("HexUtils", () => {
  ...
  describe("Range & Rings", () => {
    it("getHexesInRange (radius 0) should return only center", () => {
      const range = HexUtils.getHexesInRange(center, 0);
      expect(range).toHaveLength(1);
      expect(range[0]).toEqual(center);
    });

    it("getHexesInRange (radius 1) should return center + 6 neighbors", () => {
      const range = HexUtils.getHexesInRange(center, 1);
      expect(range).toHaveLength(7); // 1 + 6
    });

    it("getHexesInRange (radius 2) should return 19 hexes", () => {
      // Formula: 3n(n+1) + 1 => 3*2(3) + 1 = 19
      const range = HexUtils.getHexesInRange(center, 2);
      expect(range).toHaveLength(19);
    });

    it("getHexesInRing (radius 0) should return center", () => {
      const ring = HexUtils.getHexesInRing(center, 0);
      expect(ring).toHaveLength(1);
      expect(ring[0]).toEqual(center);
    });

    it("getHexesInRing (radius 1) should return 6 hexes", () => {
      const ring = HexUtils.getHexesInRing(center, 1);
      expect(ring).toHaveLength(6);
    });

    it("getHexesInRing (radius 2) should return 12 hexes", () => {
      // Ring size is usually radius * 6
      const ring = HexUtils.getHexesInRing(center, 2);
  ...
  describe("Serialization", () => {
    it("getID should return correct string format", () => {
      expect(HexUtils.getID(center)).toBe("0,0,0");
      expect(HexUtils.getID(neighborE)).toBe("1,0,-1");
    });

    it("parseID should reverse getID", () => {
      const id = "1,-2,1";
      const coord = HexUtils.parseID(id);
