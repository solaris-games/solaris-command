...
describe("CombatCalculator", () => {
  ...
  describe("calculateShifts", () => {
    ...
    it("should apply Shock Shift (Battleship vs Frigate)", () => {
      const attacker = createTestUnit(CATALOG_UNIT_BATTLESHIP_ID, 5); // Shock 2
      const defender = createTestUnit(CATALOG_UNIT_FRIGATE_ID, 5); // Shock 0
      const emptyHex = createHex(TerrainTypes.EMPTY);
    ...
    it("should apply Shock Shift with specialist", () => {
      const attacker = createTestUnit(CATALOG_UNIT_BATTLESHIP_ID, 5); // Shock 2
      const defender = createTestUnit(CATALOG_UNIT_FRIGATE_ID, 5); // Shock 0
      const emptyHex = createHex(TerrainTypes.EMPTY);

      attacker.steps[0].specialistId = "spec_shock_01"; // Shock 1
    ...
    it("should Negate Shock if Defender has Torpedoes", () => {
      const attacker = createTestUnit(CATALOG_UNIT_BATTLESHIP_ID, 5); // Shock 2
    ...
    it("should Negate Shock if hex is fortified", () => {
      const attacker = createTestUnit(CATALOG_UNIT_BATTLESHIP_ID, 5); // Shock 2
      const defender = createTestUnit(CATALOG_UNIT_FRIGATE_ID, 5);
