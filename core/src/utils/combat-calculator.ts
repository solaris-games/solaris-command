...
export const CombatCalculator = {
  ...
  getUnitPower(unit: Unit, isAttacking: boolean): number {
    ...
    let power = isAttacking 
      ? unit.stats.attack * activeSteps
      : unit.stats.defense * activeSteps;

    // Add Specialist Bonuses
    unit.steps.forEach(step => {
      if (!step.isSuppressed && step.specialist) {
        const stats = step.specialist.stats;
        power += isAttacking ? stats.attack : stats.defense;
      }
    });
  ...
  calculateShifts(attacker: Unit, defender: Unit, hex: Hex): CombatShift[] {
    ...
    if (!defenderHasTorpedo) {
      const armorDiff = (attacker.stats.armor || 0) - (defender.stats.armor || 0);
      ...
      if (armorDiff > 0) {
        shifts.push({ type: CombatShiftType.ARMOR, value: Math.min(3, armorDiff) });
      }
    } else {
   ...
   */
  getSpecialistShiftSum(unit: Unit, stat: 'artillery' | 'siege' | 'armor'): number {
    return unit.steps.reduce((sum, step) => {
      if (!step.isSuppressed && step.specialist) {
        return sum + (step.specialist.stats[stat] || 0);
      }
      return sum;
    }, 0);
  }
